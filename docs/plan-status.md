...
本表对照《数字江河 V3｜整体重绘与可视化增强正式施工方案》。V3 已从本地 `v3-freeze-candidate` 推送到 `main` 并发布到 GitHub Pages；用户提供的原始蓝图文件保留在工作区，不纳入代码入口。
...
构建产物位于 `dist/`，其内容随本地构建更新。浏览器 QA 已覆盖 390、430、768、1024、1440、1920 视口的横向溢出检查，并检查了导航目录、证据抽屉、77 节点交互、黄山步骤和真实图片自然尺寸；线上回读确认标题、8 个场景、77 个节点、来源抽屉、无横向溢出和无控制台错误。Lighthouse 与真实设备测试仍未执行。

## 收口待办

- 在最终人工审阅前，继续保留 `reported` / `target` / `pending` 状态，不把候选版称为正式冻结。
- 已完成本轮部署授权、产物回读和线上内容/控制台复核；后续发布仍需重新执行 `npm run verify` 与线上回读。
